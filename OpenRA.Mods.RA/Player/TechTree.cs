...
namespace OpenRA.Mods.RA
{
	...
	public class TechTree
	{
		...
		static Cache<string, List<Actor>> GatherBuildables(Player player)
		{
			...
			// Add buildables that have a build limit set, are also in the world, and are not already in the list
			player.World.ActorsWithTrait<Buildable>()
				  .Where(a => a.Actor.Info.Traits.Get<BuildableInfo>().BuildLimit > 0 && !a.Actor.IsDead() && a.Actor.Owner == player && a.Actor.IsInWorld && ret.Keys.All(k => k != a.Actor.Info.Name))
				  .ToList()
